...
# Оценка параметров с помощью функции garch() из пакета tseries
...
# Оценка параметров (a0, a1, a2, a3) на обучающей выборке с помощью функции garch из пакета tseries
...
garch_process_1_1 <- function(a0, a1, b1, num_obs, initial_volatility = 1, initial_process_value = 1) {
  # Проверка, что a0 положительно
  if (a0 <= 0) stop("a0 должно быть положительным")
  # Проверка, что a1 и b1 положительны, и их сумма меньше 1, что обеспечивает стационарность
  if (a1 <= 0 || a1 >= 1 || b1 <= 0 || b1 >= 1 || (a1 + b1) >= 1)
    stop("Параметры должны удовлетворять условию 0 < a1 + b1 < 1")
  
  # Инициализация векторов для хранения значений процесса и волатильности
  process_values <- numeric(num_obs) # Вектор значений процесса
  volatility <- numeric(num_obs) # Вектор для хранения значений волатильности
  
  # Генерация случайных ошибок из стандартного нормального распределения
  errors <- rnorm(num_obs, mean = 0, sd = 1)
  
  # Установка начальных значений волатильности и процесса
  volatility[1] <- initial_volatility # Начальная волатильность
  process_values[1] <- initial_process_value # Начальное значение процесса
  
  # Основной цикл для вычисления значений процесса и волатильности
  for (t in 2:num_obs) {
    # Вычисление волатильности на текущем шаге с учетом прошлого значения волатильности (b1)
    volatility[t] <- a0 + a1 * (process_values[t - 1])^2 + b1 * volatility[t - 1]
    # Значение процесса определяется как произведение корня из волатильности и случайной ошибки
    process_values[t] <- sqrt(volatility[t]) * errors[t]
  }
  
  # Разделение области построения графиков на 2 строки для отображения двух графиков
  par(mfrow = c(2, 1))
  # Построение графика значений процесса
  plot(process_values, type = 'l', col = 'seagreen',
       main = 'Стационарный процесс {h_n} GARCH(1,1)',
       ylab = 'h_n', xlab = 'Время (n)')
  # Построение графика значений волатильности
  plot(volatility, type = 'l', col = 'blue',
       main = 'Волатильность {σ_n} GARCH(1,1)',
       ylab = 'σ_n', xlab = 'Время (n)')
  
  # Возвращение значений процесса
  return(process_values)
}

# Генерация GARCH(1,1) с параметрами
n <- 1000 # Количество наблюдений
a0 <- 0.2 # Параметр a0 
a1 <- 0.3 # Параметр a1 
b1 <- 0.4 # Параметр b1 

# Вызов функции для моделирования процесса GARCH(1,1)
result_garch_1_1 <- garch_process_1_1(a0, a1, b1, n)

# Оценка параметров с помощью функции garch() из пакета tseries

# Оценка параметров (a0, a1, b1) для процесса GARCH(1,1)
garch_fit_1_1 <- garch(result_garch_1_1, order = c(1, 1), trace = FALSE)

# Извлечение оценок параметров
est_a0_garch_1_1 <- coef(garch_fit_1_1)[1] # Извлечение оценки a0
est_a1_garch_1_1 <- coef(garch_fit_1_1)[2] # Извлечение оценки a1
est_b1_garch_1_1 <- coef(garch_fit_1_1)[3] # Извлечение оценки b1

# Печать результатов оценки параметров
cat("Оцененные параметры GARCH(1,1):\n")
cat("a0:", est_a0_garch_1_1, "\n") # Вывод оценки a0
cat("a1:", est_a1_garch_1_1, "\n") # Вывод оценки a1
cat("b1:", est_b1_garch_1_1, "\n") # Вывод оценки b1
